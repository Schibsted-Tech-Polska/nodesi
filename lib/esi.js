...
ESI.prototype.include = function(node, options) {
    ...
    return new Promise(function(resolve, reject) {
        self.get(src, options).then(function(html) {
            var parsedHtml = self.parser.parseFragment(html);
            var stringHtml = self.serializer.serialize(parsedHtml);
            if(stringHtml.indexOf('include') > -1) {
                self.process(stringHtml).then(function(result) {
                    self.replace(node, self.parser.parseFragment(result));
                    resolve();
                });
